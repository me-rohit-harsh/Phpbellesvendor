...
export const safeImagePicker = async (options = {}, source = 'library') => {
  try {
    ...
    // Older Android versions also use Photo Picker if available via Google Play Services
    // We rely on expo-image-picker to handle this logic correctly without permissions
    if (Platform.OS === 'android') {
      // For Android, we skip checking permissions because we use the Photo Picker
      // which doesn't require storage permissions.
      // However, if expo-image-picker fails, we might fallback or show error.
    } else {
      // Ensure permission is granted for iOS/others
    ...
    if (source === 'camera') {
      ...
      const hasCameraPermission = await ensurePermission(PERMISSION_TYPES.CAMERA);
      if (!hasCameraPermission) return null;
      
      result = await ImagePicker.launchCameraAsync(defaultOptions);
    } else {
      // Use launchImageLibraryAsync which uses Photo Picker on Android 13+ (and 11+ with updates)
      // This avoids READ_EXTERNAL_STORAGE permission
      try {
        result = await ImagePicker.launchImageLibraryAsync(defaultOptions);
      } catch (error) {
        console.warn('ImagePicker.launchImageLibraryAsync failed, falling back to DocumentPicker', error);
        // Fallback to DocumentPicker if ImagePicker fails (e.g. on very old Android without permissions)
        if (Platform.OS === 'android') {
          ...
          if (!doc.canceled && doc.assets && doc.assets[0]) {
            ...
            console.info('🤖 Android DocumentPicker selected image:', { uri: asset.uri, type: fileType, name: fileName, size: asset.size });
    ...
    if (!result?.canceled && Array.isArray(result.assets) && result.assets.length > 0) {
      ...
      if (asset.uri) {
        const uriExtensionMatch = asset.uri.match(/\.([^./?]+)(?:\?|$)/);
      ...
      console.info('🍎 Image picker selected image:', imageFile);

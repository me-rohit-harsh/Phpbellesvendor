...
class PersistentStorage {
  ...
  static async saveData(key, data, expiryHours = 24) {
    try {
      ...
      console.log(`[PersistentStorage] Saved data for key: ${key}`);
      return true;
    } catch (error) {
  ...
  static async getData(key) {
    try {
      ...
      if (Date.now() > parsedData.expiryTime) {
        console.log(`[PersistentStorage] Data expired for key: ${key}, removing...`);
        await this.removeData(key);
        return null;
      }

      console.log(`[PersistentStorage] Retrieved data for key: ${key}`);
  ...
  static async removeData(key) {
    try {
      ...
      console.log(`[PersistentStorage] Removed data for key: ${key}`);
    } catch (error) {
  ...
  static async saveFormDraft(formId, formData) {
    try {
      ...
      console.log(`[PersistentStorage] Saved draft for form: ${formId}`);
    } catch (error) {
  ...
  static async removeFormDraft(formId) {
    try {
      ...
      console.log(`[PersistentStorage] Removed draft for form: ${formId}`);
    } catch (error) {
  ...
  static async clearAllTempData() {
    try {
      ...
      console.log('[PersistentStorage] Cleared all temporary data');
    } catch (error) {
  ...
  static async cleanupExpiredData() {
    try {
      ...
      console.log('[PersistentStorage] Cleanup completed');
    } catch (error) {

...
const convertAndroidContentUri = async (imageFile) => {
  try {
    ...
    const fileName = `temp_image_${Date.now()}.${fileExtension}`;
    ...
    return {
      ...
      type: imageFile.type || getMimeType(fileExtension),
      size: fileInfo.size,
    };
  } catch (error) {
  ...
  try {
    ...
    if (typeof imageFile === 'object' && imageFile.uri) {
      ...
      if (Platform.OS === 'android') {
        ...
        fileObj = {
          ...
          type: processedImageFile.type || `image/${fileExtension}`,
          name: processedImageFile.name || `menu_item_image.${fileExtension}`,
        };
      } else {
    ...
    const config = {
      ...
      headers: {
        ...
        // DO NOT set Content-Type manually - let Axios auto-detect FormData and set boundary
      }
      // DO NOT use transformRequest with FormData - let Axios handle it natively
    };
...
export const updateMenuItem = async (itemId, itemData) => {
  try {
    ...
    if (hasNewImage) {
      ...
      const response = await api.post(`/vendor/menu-items/${itemId}?_method=PUT`, formData, config);
      return response;
    } else {
